...
def make_movie_json_db(smlk_dir):
    ...
    #自己设置json路径--开始
    raw_data_jsons1 = glob.glob("/home/smrtanalysis/software/smrtlink_11.1.0.166339/userdata/jobs_root/0000/*/*/outputs/raw_data.report.json")
    raw_data_jsons2 = glob.glob("%s/raw_data.report.json" % smlk_dir)
    raw_data_jsons3 = glob.glob("/share/soft/smrtlink_10.2.0.133434/userdata/jobs_root/0000/0000001/*/outputs/raw_data.report.json")
    #自己设置json路径--结束
    raw_data_jsons = raw_data_jsons1 + raw_data_jsons2 + raw_data_jsons3
...
def parse_consensusreadset_xlm(xlmfile):
    ...
    try:
        ...
        time_stamp = root.getElementsByTagName('Name')[0].childNodes[0].data
        well_name = root.getElementsByTagName('WellName')[0].childNodes[0].data
        library_name = root.getElementsByTagName('WellSample')[0].getAttribute('Name')
...
def stat_reads(files, out, obspath, smlkdir):
    ...
    title2 = [
        ...
        'ZMWs', 'P0', 'P1', 'P2', 
        'ccs_reads', 'ccs_bases(bp)', 'mean_len(bp)',
        'N50_len(bp)', 'Max_len(bp)',
        'mean_pass',
        'Q20', 'Q30',
        'path'
    ]
    ...
    for file in files:
        ...
        d_raw_data = db[name]['raw_data_dict']
        d_loading = db[name]['loading_dict']
        time_stamp, well_name, library_name = db[name]['library']
        contents = [
            "{}_{}".format(time_stamp, well_name),
            library_name,
            name,
            "{:,}".format(d_raw_data['Polymerase_Read_Bases(bp)']),
            "{:,}".format(d_raw_data['Polymerase_Reads']),
            "{:,}".format(d_raw_data['Polymerase_mean_len(bp)']),
            "{:,}".format(d_raw_data['Polymerase_N50_len(bp)']),
            str(d_loading['zmws']),
            "{}({:.2%})".format(d_loading['p0'], d_loading['p0'] / d_loading['zmws']),
            "{}({:.2%})".format(d_loading['p1'], d_loading['p1'] / d_loading['zmws']),
            "{}({:.2%})".format(d_loading['p2'], d_loading['p2'] / d_loading['zmws']),
            "{:,}".format(len(length)),
            "{:,}".format(sum(length)),
            "{:,.2f}".format(avg_length),
            "{:,}".format(n50_len),
...
def add_hlep_args(parser):
    ...
    parser.add_argument('--smrtlink', metavar='STR', type=str, 
        default='/share/erapool/personal/smrtanalysis/software/smrtlink*/userdata/jobs_root/0000/*/*/outputs',
        help="smrtlink analysis dir[/share/erapool/personal/smrtanalysis/software/smrtlink_10.2.0.133434/userdata/jobs_root/0000]")
...
def main():
    ...
    stat_reads(args.input, args.out, args.obspath, args.smrtlink)
